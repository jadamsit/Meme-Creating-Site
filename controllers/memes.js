...
/* DO WORK IN THE FUNCTION BELOW FOR PART 1!
 ...
 exports.getViewData = function (userName, req){
 	return Q.all([
		Images.findAll({
							where: {
								uploaded: 1,
		...
		var userID = images1[0].userId;

 		return {
 			imageList : images1, //the array of images, accessible via imageList in memes.ejs
 			user : req.user, //the user object or a 1, accessible via user in memes.ejs
			userID : userID
 		}
 	}).catch(function (err){
...
PythonShell.run('//scripts/upload.py', function (err) {
  if (err) throw err;
  next();
});
}

exports.remUser = function(req,res,next){
	var PythonShell = require('python-shell');
	var options = {
    args: [req.body.userId]
  }
	PythonShell.run('//scripts/remUser.py', options, function (err) {
	  if (err) throw err;
	  next();
	});
}

exports.remImage = function(req,res,next){
	var PythonShell = require('python-shell');
	var options = {
    args: [req.body.imageId]
  }
	PythonShell.run('//scripts/remImage.py', options, function (err) {
	  if (err) throw err;
	  next();
	});
}
...
exports.approve = function(req,res,next){
	...
	PythonShell.run('//scripts/approve.py', options, function (err) {
	  if (err) throw err;
	  next();
	});
}
