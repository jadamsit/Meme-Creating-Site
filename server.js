...
passport.use(new StrategyGoogle({
		clientID: process.env.GOOGLE_CLIENT_ID || '743416386477-jlpe6feqbc4ru1jcnvc47vcapo610i3a.apps.googleusercontent.com',
		clientSecret: process.env.GOOGLE_CLIENT_SECRET || 'E67i_44u8F5nP-3Ow5CsXJGF',
		callbackURL: 'http://nodejs.'+ (process.env.NET_ID || 'jwadams') +'.it210.it.et.byu.edu/auth/google/return'
	},
...
app.get('/admin', function (req, res){
	memes.getAdminData(req).then(function (adminData){
...
*/
app.get('/memes/:userName/view', function (req, res){
	var userName = req.params.userName;
	memes.getViewData(userName,req).then(function (viewData){
		res.render('view', viewData);
	})
});

app.post('/approve/:imageId', memes.approve, function (req, res){
	res.redirect('/admin');
});

app.post('/remUser', memes.remUser, function(req,res){
	res.redirect('/admin');
});

app.post('/remImage', memes.remImage, function(req,res){
	res.redirect('/admin');
});
...
app.get('/meme-hook', memes.upload, function (req, res){
